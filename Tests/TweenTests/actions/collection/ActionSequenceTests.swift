...
class SequenceTests: XCTestCase {
    ...
    func testActionsEventsAreCalledInExpectedOrder() {
        ...
        func makeBecomeActiveString(tag: Int) -> String {
            return "Become Active: \(tag)"
        }

        func makeBecomeInactiveString(tag: Int) -> String {
            return "Become Inactive: \(tag)"
        }

        func makeWillBeginString(tag: Int) -> String {
            return "Will Begin: \(tag)"
        }

        func makeDidFinishString(tag: Int) -> String {
            return "Did Finish: \(tag)"
        }
        ...
        func makeAction(tag: Int) -> FiniteTimeActionMock {
            ...
            action.onBecomeActive = {
                events.append(makeBecomeActiveString(tag: tag))
            }
            action.onBecomeInactive = {
                events.append(makeBecomeInactiveString(tag: tag))
            }
            action.onWillBegin = {
                events.append(makeWillBeginString(tag: tag))
            }
            action.onDidFinish = {
                events.append(makeDidFinishString(tag: tag))
            }
            return action
        }
